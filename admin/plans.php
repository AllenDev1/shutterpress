    ...
    if ($edit_mode) {
        ...
        if ($result !== false) {
            ...
            if ($plan && $plan->woocommerce_product_id) {
                ...
                // Ensure product remains tagged as a plan product
                wp_set_post_terms($plan->woocommerce_product_id, ['shutterpress-plan-product'], 'product_tag', true);
        ...
        if ($result !== false) {
            ...
            if ($product_id) {
                ...
                wp_set_post_terms($product_id, ['shutterpress-plan-product'], 'product_tag', true);
...
function shutterpress_render_create_plan_page()
{
    ...
    <div class="wrap">
        ...
        <?php if ($edit_mode): ?>
            <p><a href="<?php echo admin_url('admin.php?page=shutterpress_all_plans'); ?>" class="button">← Back to All
                    Plans</a></p>
        <?php endif; ?>
        ...
        <form method="POST" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
            ...
            <table class="form-table">
                <tr>
                    ...
                    <td><input type="text" id="plan_name" name="plan_name"
                            value="<?php echo esc_attr($plan->plan_name ?? ''); ?>" class="regular-text" required /></td>
                </tr>
                <tr>
                    <th scope="row"><label for="plan_quota">Quota</label></th>
                    <td>
                        <input type="number" id="plan_quota" name="plan_quota"
                            value="<?php echo esc_attr($plan->quota ?? ''); ?>" class="regular-text" required />
                ...
                <tr>
                    ...
                    <td>
                        <input type="number" step="0.01" id="plan_price" name="plan_price"
                            value="<?php echo esc_attr($plan->price ?? ''); ?>" class="regular-text" required />
            ...
            <p class="submit">
                <input type="submit" class="button button-primary"
                    value="<?php echo $edit_mode ? 'Update Plan' : 'Add Plan'; ?>" />
...
function shutterpress_render_all_plans_page()
{
    ...
    <div class="wrap">
        ...
        <?php if (empty($plans)): ?>
            <div class="notice notice-info">
                <p>No subscription plans found. <a href="<?php echo admin_url('admin.php?page=shutterpress'); ?>">Create your
                        first plan</a></p>
            </div>
        <?php else: ?>
                        ...
                        <tr>
                            <td id="cb" class="manage-column column-cb check-column">
                                <input id="cb-select-all-1" type="checkbox"
                                    onclick="jQuery('.cb-plan').prop('checked', this.checked);" />
                            </td>
                        ...
                        <?php foreach ($plans as $plan): ?>
                            <tr>
                                <th scope="row" class="check-column">
                                    <input type="checkbox" name="plan_ids[]" value="<?php echo esc_attr($plan->id); ?>"
                                        class="cb-plan" />
                                </th>
                                ...
                                <td class="column-name">
                                    ...
                                    <div class="row-actions">
                                        <span class="edit">
                                            <a
                                                href="<?php echo admin_url('admin.php?page=shutterpress&edit=' . $plan->id); ?>">Edit</a>
                                            |
                                        </span>
                                        <span class="delete">
                                ...
                                <td class="column-product">
                                    <?php if ($plan->woocommerce_product_id): ?>
                                        <a href="<?php echo admin_url('post.php?post=' . $plan->woocommerce_product_id . '&action=edit'); ?>"
                                            target="_blank">
    ...
    <style>
        ...
        .wp-list-table .column-cycle {
            width: 100px;
        }

        .wp-list-table .column-unlimited {
            width: 80px;
            text-align: center;
        }
