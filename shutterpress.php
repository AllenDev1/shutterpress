...
// ✅ SIMPLE: Enable Wasabi upload only in product admin pages
...
{
    if ($key === 'copy-to-s3') {
        // Enable upload to Wasabi on product pages
...
// ✅ SIMPLE: When product is saved, force upload downloadable files and store keys
...
function shutterpress_handle_downloadable_files($post_id)
{
    ...
    if (!$product || !is_a($product, 'WC_Product') || !$product->is_downloadable()) return;

    error_log("=== PROCESSING PRODUCT $post_id ===");
    ...
    if (isset($_POST['_wc_file_urls']) && is_array($_POST['_wc_file_urls'])) {
        foreach ($_POST['_wc_file_urls'] as $file_url) {
            if (!empty($file_url)) {
                ...
                if ($attachment_id) {
                    error_log("Processing downloadable file: $file_url (ID: $attachment_id)");
                    
                    // Force upload to Wasabi
                    ...
                    if ($wasabi_key) {
                        ...
                        error_log("Stored Wasabi key for product: $wasabi_key");
                        break; // Use first file's key
                    }
                }
            }
        }
    ...
    // Double-check: Get key from actual product downloads
    ...
    if (!empty($downloads) && !get_post_meta($post_id, '_wasabi_object_key', true)) {
        ...
        if ($attachment_id) {
            ...
            if ($wasabi_key) {
                update_post_meta($post_id, '_wasabi_object_key', $wasabi_key);
                error_log("Stored Wasabi key from downloads: $wasabi_key");
            }
        }
    }
}

// ✅ SIMPLE: Force upload a file to Wasabi - DIRECT UPLOAD
function shutterpress_force_upload_file($attachment_id)
{
    ...
    error_log("Forcing upload for attachment: $attachment_id");
    ...
    if (!$access_key || !$secret_key) {
        error_log('Wasabi credentials not found');
        return false;
    }
    ...
    try {
        ...
        error_log("Uploading to Wasabi: $key");
        ...
        if ($result) {
            ...
            error_log("Successfully uploaded to Wasabi: $key");
            return true;
        }
...
// ✅ SIMPLE: Get Wasabi key for an attachment
function shutterpress_get_wasabi_key($attachment_id)
{
    ...
    // Try to get from WP Offload Media
...
function shutterpress_show_wasabi_column($column_name, $post_id)
{
    if ($column_name === 'shutterpress_wasabi') {
        ...
        if ($uploaded) {
            echo '<span style="color: green;">✓ Uploaded</span>';
        } else {
            echo '<span style="color: #666;">Local</span>';
        }
    }
}
